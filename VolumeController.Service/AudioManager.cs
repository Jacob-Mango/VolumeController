...
namespace VolumeController.Service {
	...
	public static class AudioManager {
		public static float GetVolume(int pid) {
			if (pid < 0) {
				...
				return GetApplicationVolume(pid);
			}
		}
		...
		/// </summary>
		/// <returns>-1 in case of an error, if successful the value will be between 0 and 1</returns>
		...
		public static string GetApplicationIconPath(int pid) {
			IAudioSessionControl2 ctl = GetSessionControl(pid);
			if (ctl == null)
				return "";

			string s;
			ctl.GetIconPath(out s);
		...
		private static IAudioSessionControl2 GetSessionControl(int pid) {
			...
			try {
				...
				IAudioSessionControl2 sesCtl = null;
				for (int i = 0; i < count; ++i) {
					...
					try {
						...
						if (cpid == pid) {
							sesCtl = ctl as IAudioSessionControl2;
							break;
						}
					} finally {
						//if (ctl != null)
		...
		public static List<ProcessInformation> GetAudioApplications() {
			IMMDeviceEnumerator deviceEnumerator = null;
			IAudioSessionEnumerator sessionEnumerator = null;
			IAudioSessionManager2 mgr = null;
			IMMDevice speakers = null;
			try {
				...
				List<ProcessInformation> controls = new List<ProcessInformation>();
				for (int i = 0; i < count; ++i) {
					IAudioSessionControl2 ctl = null;
					try {
						sessionEnumerator.GetSession(i, out ctl);
						...
						controls.Add(new ProcessInformation(pid, name));
					} finally {
						if (ctl != null)
							Marshal.ReleaseComObject(ctl);
					}
	...
	public struct ProcessInformation {
		...
		public ProcessInformation(int pid, string name) {
			this.processID = pid;
			this.name = name;
		}
	}
